...
def format_response(weather):
    try:
        ...
        label_text = 'City: {}\nWeather: {}\nTemperature ({}F): {}'.format(name, desc, ds, temp)
        return label_text
...
entry = tk.Entry(top_frame, font=('consolas', 18))
entry.place(relwidth=0.65, relheight=1)

# Add button to upper frame
# 1. Test the button
button = tk.Button(top_frame, text='Get Weather', font=('consolas', 18), command=lambda: get_weather(entry.get()))
button.place(relx=0.7, relwidth=0.3, relheight=1)   # Button is 25% of frame in width and height
...
bg_color = 'white'
label = tk.Label(low_frame, bg=bg_color, font=('consolas', 18), anchor='nw', justify='left', bd=5)
